/**
 * Caveman prompt templates — adapted from JuliusBrussee/caveman-code
 * (packages/coding-agent/src/core/system-prompt.ts:buildCaveModePrompt).
 *
 * Pure caveman: compression levels only. No ADHD structure rules.
 * i-have-adhd is a separate on-demand skill.
 *
 * Human-readable copy in prompts.md.
 */

export type CavemanLevel = "lite" | "full" | "ultra";

export const LEVELS: Record<CavemanLevel, string> = {
	lite: "lite",
	full: "full",
	ultra: "ultra",
};
...
export const CAVE_PROMPTS: Record<CavemanLevel, string> = {
	lite: LITE,
	full: FULL,
	ultra: ULTRA,
};
